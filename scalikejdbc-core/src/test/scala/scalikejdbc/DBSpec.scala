...
class DBSpec extends FlatSpec with Matchers with BeforeAndAfter with Settings with LoanPattern with ScalaFutures {
  ...
  it should "execute list in ioLocalTx block 2" in {
    val tableName = tableNamePrefix + "_singleInIOLocalTx"
    ultimately(TestUtils.deleteTable(tableName)) {
      TestUtils.initialize(tableName)
      import scalikejdbc.TxBoundary.IO.ioTxBoundary
      val fResult = DB fLocalTx[IO,List[Option[String]]] { s =>
  ...
  it should "execute update in ioLocalTx block 2" in {
    val tableName = tableNamePrefix + "_singleInIOLocalTx"
    ultimately(TestUtils.deleteTable(tableName)) {
      TestUtils.initialize(tableName)
      import scalikejdbc.TxBoundary.IO.ioTxBoundary
      val fCount = DB fLocalTx[IO,Int]{ s =>
      ...
      val fName = IO(firstResult).flatMap { _ =>
        DB fLocalTx[IO,Option[String]] (s => IO(s.single("select name from " + tableName + " where id = ?", 1)(rs => rs.string("name"))))
      }
  ...
  it should "not be able to rollback in ioLocalTx block 2" in {
    val tableName = tableNamePrefix + "_singleInIOLocalTx"
    ultimately(TestUtils.deleteTable(tableName)) {
      TestUtils.initialize(tableName)
      futureUsing(DB(ConnectionPool.borrow())) { db =>
        ...
        db.rollbackIfActive()
        val fName = DB fLocalTx[IO,Option[String]] { s =>
  ...
  it should "do rollback in ioLocalTx block 2" in {
    val tableName = tableNamePrefix + "_rollback"
    ultimately(TestUtils.deleteTable(tableName)) {
      TestUtils.initialize(tableName)
      import scalikejdbc.TxBoundary.IO.ioTxBoundary
      val failure = DB fLocalTx[IO,Int] { implicit s =>
        IO(s.update("update " + tableName + " set name = ? where id = ?", "foo", 1))
          .map(_ => s.update("update foo should be rolled back"))
      }
      intercept[Exception] {
